...
cm_spr = count_metric(road_map, merged, vehicles)
omm_spr, oma_spr = object_metrics(merged, objects)

print("spread")
print(cm_spr, omm_spr, oma_spr)
...
merged = merge_count_matrices(tx, vehicles, MERGE_MODE_COUNT)
cm_inc = count_metric(road_map, merged, vehicles)
omm_inc, oma_inc = object_metrics(merged, objects)

print("increasing distance")
print(cm_inc, omm_inc, oma_inc)
